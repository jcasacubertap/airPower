...
# ═══════════════════════════════════════════════════════════════════════
#  SECTION 2 — READ USER PARAMETERS FROM system/inputDomain
# ═══════════════════════════════════════════════════════════════════════

"""
    parse_inputdomain(filepath) → Dict{String,String}

Minimal parser for an OpenFOAM-style dictionary file.
Strips C/C++ comments, flattens braces, and returns key-value pairs
for every `key  value;` line encountered at any nesting level.
"""
function parse_inputdomain(filepath::String)::Dict{String,String}
    ...
    for line in split(text, '\n')
        line = strip(line)
        # Skip braces, empty lines, FoamFile header keywords
        (isempty(line) || line == "{" || line == "}") && continue
        # Match  "key   value;"
        m = match(r"^(\w+)\s+(.+?)\s*;\s*$", line)
        m === nothing && continue
        params[m.captures[1]] = m.captures[2]
    end
    return params
end

function load_params()
    filepath = joinpath(@__DIR__, "system", "inputDomain")
    p = parse_inputdomain(filepath)

    # Helper accessors
    getf(k) = parse(Float64, p[k])
    geti(k) = parse(Int,     p[k])
    getb(k) = lowercase(p[k]) == "true"

    # — Airfoil characteristics —
    global CHORD      = getf("chord")
    global ALPHA_DEG  = getf("alphaDeg")
    global X_CENTER   = getf("xAirfoilCenter")
    global Y_CENTER   = getf("yAirfoilCenter")

    # — Domain size —
    global XI_ARCH             = getf("xiArch")
    global XI_SUCTION_OUTLET   = getf("xiSuctionOutlet")
    global XI_PRESSURE_OUTLET  = getf("xiPressureOutlet")
    global H_BL       = getf("hBL")
    global H_FAR      = getf("hFar")
    global Z_WIDTH    = getf("zWidth")
    global NS_SUCTION  = geti("nSegSuction")
    global NS_ARCH_UP  = geti("nSegArchUp")
    global NS_ARCH_LO  = geti("nSegArchLo")
    global NS_PRESSURE = geti("nSegPressure")
    global COSINE_ARCH = getb("cosineArch")

    # — Grid resolution —
    global NX_TOTAL    = geti("nxTotal")
    global NY_BL       = geti("nyBL")
    global NZ          = geti("nz")
    global GRAD_BL     = getf("gradBL")
    global GRAD_ARCH   = getf("gradArch")
end
